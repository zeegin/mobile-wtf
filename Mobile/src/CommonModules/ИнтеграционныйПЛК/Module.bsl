...
Функция НовыйПараметрОтправкиHC35() Экспорт
	...
	Результат.Вставить("НомерБухты");
	Результат.Вставить("КодОператора");
...
Функция ОтправитьHC35(ПараметрОтправкиHC35) Экспорт
	...
	НомерЗаказа  = ФорматироватьНомерЗаказа(ПараметрОтправкиHC35.НомерЗаказа);
	НомерБухты   = ФорматироватьНомерБухты(ПараметрОтправкиHC35.НомерБухты);
	КодОператора = ФорматироватьКодОператора(ПараметрОтправкиHC35.КодОператора);
	
	Пакет = Новый Структура;
	Пакет.Вставить("НомерЗаказа", НомерЗаказа);
	Пакет.Вставить("НомерБухты", НомерБухты);
	Пакет.Вставить("Оператор", КодОператора);
...
Функция ФорматироватьНомерЗаказа(Строка)
	
	Возврат Формат(Строка, "ЧЦ=11; ЧРД=.; ЧРГ=; ЧН=0; ЧВН=; ЧГ=0;");
	
КонецФункции

Функция ФорматироватьКодОператора(Строка)
	
	Возврат Формат(Строка, "ЧЦ=9; ЧРД=.; ЧРГ=; ЧН=0; ЧВН=; ЧГ=0;");
	
КонецФункции

Функция ФорматироватьНомерБухты(Строка)
