Функция ПодключениеКВебСервису(ПараметрДляОтправкиНаСервер) Экспорт 
	ВСОпределение = Новый WSОпределения("http://192.168.1.24/PLK/ws/DataTransfer.1cws?wsdl","Admin");
	...
	Ответ = СериализаторXDTO.ПрочитатьXDTO(ВСПрокси.Registration(СериализаторXDTO.ЗаписатьXDTO(ПараметрДляОтправкиНаСервер)));
...
Функция ПодключениеКВебСервисуПЛК(ПараметрДляОтправкиНаСервер) Экспорт 
	...
	//Пароль = "";
	//
	//ВСОпределение = Новый WSОпределения("http://127.0.0.1/MP_WebService/ws/DataTransfer?wsdl", Логин, Пароль);
	//
	ВСервис 	= ВСОпределение.Сервисы.Получить("DataTransfer","DataTransfer");
	ВТочкаВхода = ВСервис.ТочкиПодключения.Получить("DataTransferSoap");
